...
TEST(ParserLib_test, tokenise) {
  ...
  EXPECT_TRUE(ifExpected == tokenise(ifStatement, 17));

  const std::string assignment {"r1 = 3 + r5"};
  std::vector<std::string> assignmentExpected {"r1", "=", "3", "+", "r5"};
  EXPECT_TRUE(assignmentExpected == tokenise(assignment, 11));
}
...
TEST(ParserLib_test, getLineLength) {
  ...
  EXPECT_EQ (8, getLineLength(normal, mockLineNumber));
  
  std::string whitespace { "string 1 " };
  EXPECT_EQ (8, getLineLength(whitespace, mockLineNumber));
  
  std::string comment { "string # 111" };
  EXPECT_EQ (6, getLineLength(comment, mockLineNumber));
}
...
TEST(ParserLib_test, parseReturn) {
  ...
  EXPECT_EQ (8, parseReturn(tokensSingle, mockLineNumber)->exec(0, r));
  
  std::vector<std::string> tokensDouble {"5.", "return", "r18"};
  EXPECT_EQ (18, parseReturn(tokensDouble, mockLineNumber)->exec(0, r));
}
...
TEST(ParserLib_test, parseGoto) {
  ...
  EXPECT_EQ (8, parseGoto(tokensSingle, mockLineNumber)->exec(0, r));
}
